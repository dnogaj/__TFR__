...
class GameParameters:
    ...
    #RACE_TRACK_BORDER_MASK_TRIM = pygame.mask.from_surface(RACE_TRACK_BORDER_TRIM)
    ...
    RACE_END_WIN_ALGO = resize_img(pygame.image.load(RACE_END_FINISH_WIN_ALGO_PATH), 2.2)
    ...
    # todo -> próba przesunięcia trasy zakończona fiaskiem (zmienne z 'TRIM') - wywala błąd przy algo 2 przy random.choice() (set czy jakoś tak), a przy algo 1 puszcza ale się chyba od razu colliduje z obramówką trasy -> to check odkomentuj wszystkie TRIM oraz FITTING w powyższym kodzie oraz patrz linia 86/87 w module game_main.py (car.collision())
